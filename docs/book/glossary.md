...
- Status
  - The datetime/duration types, literals (`d"…"`, `7d`/`30ms`/`1.5h`), truthiness, the temporal operators, the datetime function library (attached via `uexl.WithDatetime()`), and clock-injected `now`/`today` are all implemented and conformance-verified. See the [DateTime Specification](../specs/datetime-spec.md).
