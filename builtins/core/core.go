// Package core holds the always-on built-in functions — the irreducible set every UExL embedding has
// without attaching any standard-library family. The runtime assembles its default registry (vm.Builtins)
// from this map. Like the other builtins/<family> packages, it depends only on builtins/fn (+ internal
// helpers), never on vm, so the import direction stays vm -> builtins/core. See ADR-0001 and
// builtins-implementation-plan.md §3.1.
package core
...
// Builtins maps the core function names to their implementations. These are always available; the
// attachable families (math, strings, …) live in the sibling builtins/<family> packages.
